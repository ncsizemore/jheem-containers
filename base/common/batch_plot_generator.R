...
# BASE IMAGE VERSION: Auto-detects workspace file for any model
...
# AUTO-DETECT WORKSPACE: Find and load the workspace file
cat("Auto-detecting workspace file...\n")
workspace_files <- list.files(pattern = "_workspace\\.RData$", ignore.case = TRUE)
if (length(workspace_files) == 0) {
  stop("No workspace file found (expected *_workspace.RData)")
} else if (length(workspace_files) > 1) {
  stop("Multiple workspace files found: ", paste(workspace_files, collapse = ", "),
       ". Expected exactly one.")
}
workspace_file <- workspace_files[1]
load(workspace_file)
cat("Workspace loaded:", workspace_file, "\n")
cat("Objects loaded:", length(ls()), "\n")

# Report which specification objects are available
spec_objects <- c("RW.SPECIFICATION", "CDCT.SPECIFICATION", "CROI.SPECIFICATION")
available_specs <- spec_objects[sapply(spec_objects, exists)]
if (length(available_specs) > 0) {
  cat("Specification available:", paste(available_specs, collapse = ", "), "\n")
} else {
  cat("Warning: No known specification object found\n")
}

# Report which data managers are available
dm_objects <- c("WEB.DATA.MANAGER", "RW.DATA.MANAGER", "CDCT.DATA.MANAGER")
available_dms <- dm_objects[sapply(dm_objects, exists)]
if (length(available_dms) > 0) {
  cat("Data manager available:", paste(available_dms, collapse = ", "), "\n")
} else {
  cat("Warning: No known data manager found\n")
}

# Restore jheem2 internal state from workspace
...
# Set up data manager function
# Prioritizes WEB.DATA.MANAGER, then falls back to model-specific managers
get.default.data.manager <- function() {
  ...
  } else if (exists("CDCT.DATA.MANAGER", envir = .GlobalEnv)) {
    return(CDCT.DATA.MANAGER)
  } else {
